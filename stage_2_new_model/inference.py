import sys
import os
import pandas as pd
import torch
import numpy as np
import matplotlib.pyplot as plt
import time
from sklearn.preprocessing import RobustScaler

# Thêm đường dẫn để import các module tùy chỉnh
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '..')))
from stage_1.KAMA import kama_decomposition
from stage_1.STL import stl_decomposition
from stage_2_new_model.AE import TimeSeriesAutoencoder

def load_he_kan_model(model_path, device='cpu'):
    """Tải mô hình HE-KAN đã được huấn luyện."""
    checkpoint = torch.load(model_path, map_location=device, weights_only=False)
    config = checkpoint['model_config']
    model = TimeSeriesAutoencoder(
        input_dim=config.get('input_dim', 1),
        kan_out_features=config.get('kan_out_features', 4),
        gru_hidden_dim=config.get('gru_hidden_dim', 8),
        num_experts=config.get('num_experts', 8)
    )
    model.load_state_dict(checkpoint['model_state_dict'])
    model.eval()
    model.to(device)
    return model

# --- CÁC HÀM TIỆN ÍCH ---
def load_ground_truth_labels(place_id, labels_dir):
    # (Giữ nguyên)
    label_file = os.path.join(labels_dir, f'label_{place_id}.csv')
    if os.path.exists(label_file):
        labels_df = pd.read_csv(label_file)
        labels_df['is_anomaly'] = labels_df['label']
        labels_df['date'] = pd.to_datetime(labels_df['date'])
        return labels_df
    return None

def group_consecutive_indices(indices, max_gap=1):
    """Nhóm các chỉ số liên tiếp thành các cụm"""
    if len(indices) == 0:
        return []
    
    groups = []
    start = indices[0]
    end = indices[0]
    
    for i in range(1, len(indices)):
        if indices[i] - end <= max_gap + 1:
            end = indices[i]
        else:
            groups.append((start, end))
            start = indices[i]
            end = indices[i]
    
    groups.append((start, end))
    return groups

def filter_anomaly_groups(anomaly_indices, min_group_size=4):
    """
    Lọc bỏ các nhóm anomaly có ít hơn min_group_size điểm liên tiếp
    """
    if len(anomaly_indices) == 0:
        return np.array([])
    
    # Nhóm các chỉ số liên tiếp
    anomaly_groups = group_consecutive_indices(anomaly_indices, max_gap=1)
    
    # Lọc ra các nhóm đủ lớn
    filtered_indices = []
    for start_idx, end_idx in anomaly_groups:
        group_size = end_idx - start_idx + 1
        if group_size >= min_group_size:
            # Thêm tất cả chỉ số trong nhóm này
            for idx in range(start_idx, end_idx + 1):
                if idx in anomaly_indices:  # Đảm bảo nó ban đầu là anomaly
                    filtered_indices.append(idx)
    
    return np.array(filtered_indices)

# --- HÀM VẼ BIỂU ĐỒ (Cập nhật để hiển thị lỗi cho cả a và d) ---
def plot_unified_results(
    place_id, time_series, dates,
    trend_anomaly_indices, point_anomaly_indices,
    ground_truth_labels=None, error_a=None, error_d=None,
    threshold_a=None, threshold_d=None, save_path=None
):
    fig, (ax1, ax2, ax3) = plt.subplots(3, 1, figsize=(20, 18), sharex=True)
    
    # BIỂU ĐỒ 1: CHUỖI THỜI GIAN GỐC
    ax1.plot(dates, time_series, 'b-', linewidth=1, alpha=0.7, label='Original Time Series')
    
    # Vẽ Ground Truth (vàng nhạt)
    if ground_truth_labels is not None:
        gt_anomaly_dates = ground_truth_labels[ground_truth_labels['is_anomaly'] == 1]['date']
        for i, gt_date in enumerate(gt_anomaly_dates):
            ax1.axvspan(gt_date - pd.Timedelta(days=0.5), gt_date + pd.Timedelta(days=0.5),
                       alpha=0.4, color='yellow',
                       label='Ground Truth Anomaly' if i == 0 else "")
    
    # Vẽ Trend Anomalies (vàng đậm với nhóm)
    if len(trend_anomaly_indices) > 0:
        anomaly_groups = group_consecutive_indices(trend_anomaly_indices, max_gap=2)
        for i, (start_idx, end_idx) in enumerate(anomaly_groups):
            start_date = dates.iloc[max(0, start_idx-1)]
            end_date = dates.iloc[min(len(dates)-1, end_idx+1)]
            ax1.axvspan(start_date, end_date,
                       alpha=0.6, color='orange',
                       label='Predicted Trend Anomaly' if i == 0 else "")
    
    # Vẽ Point Anomalies (đỏ)
    if len(point_anomaly_indices) > 0:
        ax1.scatter(dates.iloc[point_anomaly_indices], time_series[point_anomaly_indices],
                   color='red', s=100, zorder=5, label='Predicted Point Anomaly', edgecolors='black')
    
    ax1.set_title(f'Time Series Anomaly Detection Results - PlaceId: {place_id}', fontsize=16)
    ax1.set_ylabel('View Count', fontsize=12)
    ax1.legend(fontsize=12)
    ax1.grid(True, alpha=0.3)

    # BIỂU ĐỒ 2: LỖI TÁI TẠO ABSOLUTE ERROR (A COMPONENT)
    if error_a is not None:
        ax2.plot(dates, error_a, 'g-', linewidth=1, alpha=0.7, label='Absolute Reconstruction Error (A)')
        if threshold_a is not None:
            ax2.axhline(y=threshold_a, color='orange', linestyle='--', linewidth=2, 
                       label=f'Trend Anomaly Threshold ({threshold_a:.4f})')
        if len(trend_anomaly_indices) > 0:
            anomaly_groups = group_consecutive_indices(trend_anomaly_indices, max_gap=2)
            for i, (start_idx, end_idx) in enumerate(anomaly_groups):
                start_date = dates.iloc[max(0, start_idx-1)]
                end_date = dates.iloc[min(len(dates)-1, end_idx+1)]
                ax2.axvspan(start_date, end_date,
                           alpha=0.6, color='orange',
                           label='Predicted Trend Anomaly' if i == 0 else "")
        ax2.set_title('Absolute Reconstruction Error for Trend Anomalies (A component)', fontsize=16)
        ax2.set_ylabel('Absolute Error', fontsize=12)
        ax2.legend(fontsize=12)
        ax2.grid(True, alpha=0.3)

    # BIỂU ĐỒ 3: LỖI TÁI TẠO MSE (D COMPONENT)
    if error_d is not None:
        ax3.plot(dates, error_d, 'm-', linewidth=1, alpha=0.7, label='MSE Reconstruction Error (D)')
        if threshold_d is not None:
            ax3.axhline(y=threshold_d, color='red', linestyle='--', linewidth=2, 
                       label=f'Point Anomaly Threshold ({threshold_d:.4f})')
        if len(point_anomaly_indices) > 0:
            ax3.scatter(dates.iloc[point_anomaly_indices], error_d[point_anomaly_indices], 
                       color='red', s=100, zorder=5, edgecolors='black')
        ax3.set_title('MSE Reconstruction Error for Point Anomalies (D component)', fontsize=16)
        ax3.set_ylabel('MSE Error', fontsize=12)
        ax3.legend(fontsize=12)
        ax3.grid(True, alpha=0.3)

    # Cài đặt chung
    ax3.set_xlabel('Date', fontsize=12)
    plt.setp(ax3.xaxis.get_majorticklabels(), rotation=45)
    plt.tight_layout()
    if save_path:
        plt.savefig(save_path, dpi=300, bbox_inches='tight')
    plt.close()

# --- HÀM SUY LUẬN CHÍNH (LOGIC ĐÃ ĐƯỢC ĐỒNG BỘ) ---
def inference_on_places(data_path, labels_dir, model_path, num_places=30):
    df = pd.read_csv(data_path)
    place_ids = df['placeId'].unique()[:num_places]
    
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model = load_he_kan_model(model_path, device)
    
    output_dir = 'inference_results_he_kan'
    os.makedirs(output_dir, exist_ok=True)
    
    trend_percentile = 98.0
    point_percentile = 97.0

    for i, place_id in enumerate(place_ids):
        print(f"\nProcessing place {i+1}/{len(place_ids)}: {place_id}")
        
        place_data = df[df['placeId'] == place_id].copy().sort_values('date').reset_index(drop=True)
        time_series = place_data['view'].values
        dates = pd.to_datetime(place_data['date'])
        
        ground_truth = load_ground_truth_labels(place_id, labels_dir)
        
        # Tiền xử lý
        scaler = RobustScaler()
        time_series_scaled = scaler.fit_transform(time_series.reshape(-1, 1)).flatten()
        a_np, d_np_original = stl_decomposition(time_series_scaled, period=7, robust=True)
        _, d_np_kama = kama_decomposition(time_series_scaled)
        
        # Sử dụng d_np từ KAMA cho đầu vào mô hình vì nó chứa các đỉnh nhọn tốt hơn
        a_tensor = torch.FloatTensor(a_np.copy()).unsqueeze(0).unsqueeze(-1).to(device)
        d_tensor = torch.FloatTensor(d_np_kama.copy()).unsqueeze(0).unsqueeze(-1).to(device)
        
        # Forward pass
        with torch.no_grad():
            reconstructed_a, reconstructed_d = model(a_tensor, d_tensor)
            reconstructed_a_np = reconstructed_a.squeeze().cpu().numpy()
            reconstructed_d_np = reconstructed_d.squeeze().cpu().numpy()

        ### LOGIC PHÁT HIỆN ANOMALY ĐÃ ĐỒNG BỘ ###

        # Tính absolute error cho a và MSE loss cho d
        error_a = np.abs(a_np - reconstructed_a_np)
        loss_fn = torch.nn.MSELoss(reduction='none')
        error_d = loss_fn(reconstructed_d, d_tensor).squeeze().cpu().numpy()
        
        # 1. Phát hiện Trend Anomalies (từ absolute error của 'a')
        threshold_a = np.percentile(error_a, trend_percentile)
        trend_anomaly_indices_raw = np.where(error_a >= threshold_a)[0]
        
        # Lọc bỏ nhóm trend anomalies có ít hơn 4 điểm liên tiếp
        trend_anomaly_indices = filter_anomaly_groups(trend_anomaly_indices_raw, min_group_size=4)
        
        # 2. Phát hiện Point Anomalies (từ MSE của 'd')
        threshold_d = np.percentile(error_d, point_percentile)
        point_anomaly_indices = np.where(error_d >= threshold_d)[0]
        
        print(f"  - Raw trend anomalies ({trend_percentile}th percentile on absolute error): {len(trend_anomaly_indices_raw)} points")
        print(f"  - Filtered trend anomalies (≥4 consecutive points): {len(trend_anomaly_indices)} points")
        print(f"  - Point anomalies ({point_percentile}th percentile on MSE error): {len(point_anomaly_indices)} points")

        # Vẽ biểu đồ
        plot_path = os.path.join(output_dir, f'unified_{i+1}_{place_id}.png')
        plot_unified_results(
            place_id, time_series, dates,
            trend_anomaly_indices, point_anomaly_indices,
            ground_truth, error_a, error_d,
            threshold_a, threshold_d, plot_path
        )

    print(f"\nInference completed! Results saved to '{output_dir}' directory.")


if __name__ == "__main__":
    DATA_PATH = 'data/cleaned_data_no_zero_periods_filtered.csv'
    LABELS_DIR = 'new_labels_2'
    MODEL_PATH = 'saved_models_new_model/autoencoder_model.pth'
    
    inference_on_places(DATA_PATH, LABELS_DIR, MODEL_PATH, num_places=30)
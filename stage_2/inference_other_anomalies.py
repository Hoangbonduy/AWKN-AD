...
def percentile_anomaly_detection(reconstruction_errors, percentile=99):
    """
    ...
    Args:
        ...
        percentile: Percentile threshold for anomaly detection (default 99)
    
    Returns:
        threshold: The percentile threshold value
        anomaly_indices: Indices where error >= threshold
    """
    ...
    # Find anomaly indices (use >= instead of > to include values at threshold)
    anomaly_indices = np.where(errors >= threshold)[0]
    
    return threshold, anomaly_indices

def detect_anomalies(reconstruction_loss, percentile=99):
    """Detect anomalies based on reconstruction loss using percentile threshold"""
    threshold, anomaly_indices = percentile_anomaly_detection(
...
def plot_results(place_id, time_series, dates, anomaly_indices, ground_truth_labels=None, loss_d=None, percentile_threshold=None, save_path=None):
    ...
    if ground_truth_labels is not None:
        ...
        for gt_date in gt_anomaly_dates:
            if gt_date in dates.values:
                ...
                if len(idx) > 0:
                    idx = idx[0]
                    ax1.axvspan(dates.iloc[max(0, idx-1)], dates.iloc[min(len(dates)-1, idx+1)], 
    ...
    if len(anomaly_indices) > 0:
        ax1.scatter([dates.iloc[i] for i in anomaly_indices], 
    ...
    # Plot reconstruction loss_d
    if loss_d is not None:
        ax2.plot(dates, loss_d, 'g-', linewidth=1, alpha=0.7, label='Reconstruction Loss (D component)')
        
        # Plot 99th percentile threshold line
        if percentile_threshold is not None:
            ax2.axhline(y=percentile_threshold, color='red', linestyle='--', linewidth=2, 
                       label=f'99th Percentile Threshold ({percentile_threshold:.3f})')
        
        # Plot ground truth anomalies (yellow background) on loss plot too
        if ground_truth_labels is not None:
            gt_anomaly_dates = ground_truth_labels[ground_truth_labels['is_anomaly'] == 1]['date']
            for gt_date in gt_anomaly_dates:
                if gt_date in dates.values:
                    ...
                    if len(idx) > 0:
                        idx = idx[0]
                        ax2.axvspan(dates.iloc[max(0, idx-1)], dates.iloc[min(len(dates)-1, idx+1)], 
                                   alpha=0.3, color='yellow', label='Ground Truth Anomaly' if gt_date == gt_anomaly_dates.iloc[0] else "")
        
        # Plot predicted anomalies on loss plot (red dots)
        if len(anomaly_indices) > 0:
            ax2.scatter([dates.iloc[i] for i in anomaly_indices], 
                       [loss_d[i] for i in anomaly_indices], 
                       color='red', s=100, zorder=5, label='Detected Anomaly')
        
        ax2.set_title(f'Reconstruction Loss (D component) - PlaceId: {place_id}', fontsize=16)
...
def inference_on_places(data_path, labels_dir, model_path, num_places=30):
    ...
    output_dir = 'inference_results_other_anomalies'
    ...
    for i, place_id in enumerate(place_ids):
        ...
        with torch.no_grad():
            ...
            # Ensure loss_a and loss_d are 1D arrays with correct length
            ...
            if loss_d.ndim == 0:  # scalar case
                loss_d = np.array([loss_d])
            if len(loss_d) == 1 and len(time_series) > 1:
                # If loss_d is single value but time series is longer, repeat it
                loss_d = np.full(len(time_series), loss_d[0])
        forward_time = time.time() - start_time
        total_forward_time += forward_time

        # Detect anomalies using 98th percentile of loss_d (TIMED)
        start_time = time.time()
        
        # Phát hiện anomalies với 99th percentile threshold
        anomaly_indices, percentile_threshold = detect_anomalies(
            loss_d,
            percentile=99
        )
        ...
        print(f"  Reconstruction errors (loss_d): min={loss_d.min():.6f}, max={loss_d.max():.6f}, mean={loss_d.mean():.6f}")
        print(f"  99th percentile threshold: {percentile_threshold:.6f}")
        print(f"  Anomalies detected: {len(anomaly_indices)} points ({len(anomaly_indices)/len(loss_d)*100:.2f}%)")

        # Plot results
        plot_path = os.path.join(output_dir, f'test_{i+1}_{place_id}.png')
        plot_results(place_id, time_series, dates, anomaly_indices, ground_truth, loss_d, percentile_threshold, plot_path)
        ...
        results.append({
            ...
            'avg_reconstruction_loss_a': np.mean(loss_a),
            'avg_reconstruction_loss_d': np.mean(loss_d),
    ...
    print(f"Detection Method: 99th Percentile Threshold on Loss_D")
    print(f"Percentile Thresholds Used:")
    for _, row in results_df.iterrows():
        print(f"  Place {row['place_id']}: Length={row['series_length']}, Threshold={row['percentile_threshold']:.6f}")
